...
当前仓库正式纳入的是数据与研究链路。连接券商的交易执行系统作为可选下游存在；在目标持仓交接契约稳定并完成 paper / dry-run 联调前，不将其锁定为本工作区的必选组成部分。
...
| [`cross-sectional-trees`](cross-sectional-trees/) | 截面策略研究、特征工程、模型评估、历史回测、候选治理、持仓与执行前分配分析 | `summary.json`、`positions_current*.csv`、snapshot / alloc 输出 |
...
`quant-execution-engine` 目前只作为平台链路的可选下游列出，不是本仓库的 submodule。当前 `cross-sectional-trees` 的公开输出与执行引擎的标准化 `targets.json` 输入之间仍需建立正式、可验证的交接契约。

## 典型路径

1. 使用数据平台或现有数据维护工具发布版本化的日线、PIT、估值和盘口派生资产。
2. 由 `cross-sectional-trees` 读取已发布资产，开展特征研究、模型评估、回测和候选策略治理。
3. 生成持仓快照与执行前分配结果，作为研究链路的正式终点。
4. 如需连接真实或模拟券商，先通过明确的目标持仓转换与人工门禁，将结果交给可选的执行引擎。
...
- `cross-sectional-trees` 当前负责研究到持仓 / 分配输出；券商侧执行必须经显式交接和执行系统自身的安全门禁。
- 在目标持仓契约与 paper 验证稳定之前，不在顶层加入自动触发真实交易的中央调度逻辑。
