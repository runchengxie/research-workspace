...
3. 当前 A 股工作区只保留恢复证据、删除门禁、必要的共享执行契约和恢复控制面。
4. 已完成第一批低风险入口清理；剩余港股业务模块继续按 manifest gate 逐步处理。
...
- `docs/deprecations.md` 记录 `hkdata`、`rqdata-hk-*`、`cstree alloc-hk` 等兼容入口的删除状态。
...
- `market-data-platform`: `hk_assets`、`hk_depth`、`configs/presets/release/hk_*.yml`、`configs/presets/universe/hk_*.yml`；旧 `hk_data_platform` 包和 `rqdata-hk-*` console scripts 已在活跃包移除。
...
| `hkdata`、`rqdata-hk-*`、`cstree alloc-hk` | removed compatibility entrypoints | 活跃入口已删除；归档标签保留恢复路径 |
...
3. 删除废弃 CLI / 包装入口：`hkdata`、`rqdata-hk-*`、`cstree alloc-hk`。
4. 对剩余底层业务模块运行 consumer audit，确认没有 repo-local 或下游调用。
5. 满足 deletion gate 后删除剩余业务代码。

已删除的入口在 `docs/deprecations.yml` 中标记为 `removed`。剩余底层业务模块删除前仍必须由对应 manifest 证明 gate ready。private staging 或 public demo 成功都不是删除充分条件。
...
- `hkdata`
- `rqdata-hk-*` / `rqdata-tick` console scripts
...
当前 manifest 中剩余底层业务模块仍有 `blocked_pending_audit` 或 `follow_up_required` 记录，所以不应继续直接删除港股业务代码。
...
uv run python -m pytest tests/test_transitions.py tests/test_hk_depth.py tests/test_paths.py tests/test_quality_governance.py -q
```
...
uv run python -m pytest tests/test_cli_liveops.py tests/test_docs_contracts.py tests/test_alloc_hk.py -q
```
...
3. 把 active A 股 workspace 的 CI、release checklist 继续收敛到 A 股。
4. 逐个推进剩余 manifest gate，把 `blocked_pending_audit` / `follow_up_required` 变成 `ready` 后再删除。
