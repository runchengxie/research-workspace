...
| 层 | 目录/仓库 | 定位 | 门槛 |
| --- | --- | --- | --- |
| 算法框架 | 六个子模块 + 顶层 `src/` | 长期资产，稳定演进，有治理约束 | 生产代码质量与治理 |
| 长期策略 | `strategies/` | 已验证、值得跟踪、未完全生产化 | 验证证据（见下方升格门槛） |
| 短期探索 | `experiments/` | 一次性想法与结论记录 | 无门槛，快速试错 |
...
## 从 experiments 升格到 strategies 的门槛

一个探索从 `experiments/` 升格到 `strategies/` 需要同时满足：
...
## 从 strategies 升格到 production 的门槛
...
- 维护性治理：`strategies/` 默认不计入根仓库活跃债预算（同 `experiments/`）。
