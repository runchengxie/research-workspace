...
def _factor_definition_lines() -> list[str]:
    return [
        ...
        "| Earnings Yield 盈利估值 | 多-空 | 1/PE_TTM，月度分层（价值组，估值代理而非盈利质量） |",
        "| Momentum 动量 | 多-空 | 21日收益（跳过T日），月度分层 |",
        "| Quality 复合质量 | 多-空 | 等权复合 ROE、低资产负债率、盈利稳定性、现金流质量"
        "（OCF/净利润），各子指标截尾+z 后合成 |",
...
def _industry_neutralization_note() -> list[str]:
    return [
        "",
        "## 行业中性化（申万 PIT L1）",
        "",
        "所有因子在合成 z-score 前，先按**申万一级行业（L1）**在每期截面内做行业内 demean，"
