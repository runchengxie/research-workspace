...
扩大 A 股下载范围前，先按 [data-transition-playbook.md](data-transition-playbook.md) 完成 `DATA_PLATFORM_ROOT`、current contract、registry、`daily_clean` 质量门禁和 `default_next` smoke 验证。旧称 `metadata/current_assets/cn_current.json` 只能作为历史兼容 alias 说明，不能作为新流程的权威入口。

## 港股生命周期边界

中国香港市场数据资产继续由 `market-data-platform` 生产、检查、发布和归档；`cross-sectional-trees` 只读消费。港股策略研究在当前工作区中定位为历史研究线：保留历史复现、跨市场 sanity check、少量明确资金/模拟盘/人工跟踪需求和 provenance，不作为新增研究默认入口。独立港股研究线的候选迁移清单见 [`hk-research-lane-inventory.json`](hk-research-lane-inventory.json)；其中 `quant-execution-engine` 的标准 `targets.json`、FX、broker adapter、风控和审计逻辑始终保留在执行仓库。长期不使用时，港股资产整体移入独立冷存储，活跃根目录只保留 `metadata/frozen_markets/hk.json`。需要港股专项 `alloc-hk`、历史复现或明确跟踪时，先运行 `marketdata migration hydrate-hk` 恢复资产。
