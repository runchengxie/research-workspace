...
| `research-run.manifest.json` | `research.backtest-run.v1` | `strategy-pipeline-internal` 生成 | `strategy_pipeline_internal.research_evidence`、`research_contracts.research_run_manifest` | 根运行 lineage、证据等级、研究时钟和 artifact hash |
| `targets.json.lineage.json` | target export lineage | `strategy-pipeline-internal` | `strategy export-targets` | run id、输入持仓文件、配置、质量检查、导出时间和 `artifact_envelope` |
