...
中国香港市场数据资产继续由 `market-data-platform` 生产、检查、发布和归档；`cross-sectional-trees` 只读消费。港股策略研究在当前工作区中定位为 legacy research lane：保留历史复现、跨市场 sanity check、少量明确资金/模拟盘/人工跟踪需求和 provenance，不再作为新增研究默认入口。长期不使用时，港股资产整体移入独立冷存储，活跃根目录只保留 `metadata/frozen_markets/hk.json`。需要港股专项 `alloc-hk`、历史复现或明确跟踪时，先运行 `marketdata migration hydrate-hk` 恢复资产。
...
- `frozen_markets/hk.json` 表示港股已按计划移出活跃根目录，不是 contract 意外缺失。
