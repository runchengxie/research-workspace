...
- Git submodule gitlink：workspace 模式下的源码组合版本。
...
- `version_graph.json`：workspace gitlink 与各 repo standalone Git pin 的版本图。
...
- source archive 没有 `.git`，因此无法读取 gitlink；
- 子仓 standalone Git pin 与 workspace gitlink 不同。

最后一项目前故意不作为失败条件。workspace gitlink 和 repo-local pin 服务于两种不同的可复现模式；在正式统一 resolution policy 之前，把差异先变成可见证据，比擅自重写 lockfile 更安全。
