...
本页说明顶层 `src/style_factors` 的用途、运行方式和输出约定。它服务于 A 股研究复盘和策略收益归因，正式输出目录是：
...
`style_factors` 是一个 Barra CNE5 inspired 的全市场风格 proxy 分析工具。它从
...
每个交易日做截面 winsorize 和 z-score。每个月最后一个交易日按因子分成 5 组，等权持有到下个月末，输出 top quintile long - bottom quintile short 的日收益序列。

本工具不是完整商用 Barra 风险模型：当前没有行业中性化、协方差风险预测、特异风险建模、组合优化或 PIT 指数成分约束。它适合做市场风格复盘、候选策略收益归因和研究解释，不适合作为账户级风险引擎。
...
| 文件 | 内容 |
| --- | --- |
| `factor_summary.json` | 全样本因子年化收益、波动、Sharpe、回撤和胜率 |
| `factor_correlation.json` | 因子 long-short 日收益相关性 |
| `factor_yearly.csv` | 因子逐年收益、波动、Sharpe 和回撤 |
| `factor_<name>_daily.csv` | 单个因子的 long-short 日收益 |
...
- 估值类输入中，非正 PB/PE_TTM 会被视为缺失，不会被裁剪成极端低估值或高质量。
...
- 因子收益是全市场等权 long-short proxy，没有交易成本、涨跌停成交约束、ST 过滤或行业中性化。
