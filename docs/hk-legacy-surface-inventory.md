...
| `configs/experiments/*hk*`、`docs/research/notes/hk-*` | `archived_provenance` | source tag、archive manifest 与 restore drill 完成后再迁出活跃 lane |
...
两个远端 freeze release 已下载并通过 SHA-256 与 zstd 校验，研究产物已完成隔离 hydrate
apply；数据平台归档的隔离解包和 hydrate apply 仍待完成。因此港股 research-only
provenance 仍留在活跃研究仓库中。
