...
当前版本不是生产版本，也不改变 Git 子模块指针。它记录 public/private staging 的可回滚提交和当前
...
| 日期 | 顶层仓库提交 | market-data-platform | alpha-research | portfolio-backtester | strategy-app | strategy-pipeline | quant-execution-engine | 验证状态 | 证据 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 2026-09-05 | 本次提交 | `51e5831` | `ed85f26` | `f19e5fc` | `0773206` | `5f7f868` | `d59d76d` | public pipeline 已完成 production cutover。alpha、strategy-research、strategy-app 和 quant-execution-engine 的 internal 退役边界切片已合并并同步 gitlink。各 owner PR 已通过对应检查，workspace 只保留迁移与历史审计引用 | PR #73、PR #113、PR #101、PR #29，`docs/evidence/production-cutover-20260905.md` |
| 2026-08-30 | 本次提交 | `10afd6f` | `96e33cd` | `d746e24` | `7b589c9` | `d1698d4` | `1f07cf1` | 基本面族 Shadow owner PR #80/#48/#9 已合并并同步 gitlink。A1 7 项、A2 15 项、strategy-research 179 项测试通过，生产 preset 与自动晋级保持关闭 | `docs/roadmap.md`、`strategy-research/experiments/fundamental_family_shadow/` |
| 2026-08-17 | 本次提交 | `e9c4994` | `405f712` | `1428a72` | `87064c2` | `f2b35e7` | `f0b9086` | DailyWatch20 候选池保持 `ths_hot_strict_v3`，同时完成策略边界 R6 收口和 style_factors 三层迁移。因子内核归 alpha，回测内核归 portfolio，研究呈现归 strategy-research，pipeline 只保留控制面 | alpha-research 285 项、portfolio-backtester 396 项、strategy-research 43 项测试通过。strategy-app 188 项测试和 strategy-pipeline PR #68 完整门禁通过。顶层严格门禁、研究层质量门、workspace doctor 和契约检查通过。 |
| 2026-08-16 | 本次提交 | `908f670` | `9aafbbb` | `45b7c0b` | `6555477` | `a4fcb60` | `f0b9086` | strategy-app 类型门禁恢复全绿：修复 R4 迁移带入的 21 个 `ty` 诊断、补齐 matplotlib 依赖与 wheel smoke campaign spec 登记，完整门禁通过。R6 状态改回进行中，早期策略档案索引已建立 | strategy-app PR #28 完整门禁 `All local checks passed`，`ty check` 0 诊断，pytest 188 passed，隔离 wheel smoke 通过。顶层 `docs/owner-native-namespace-release.json` 与 maintainability baseline 已同步，`docs/evidence/maintainability/baseline-20260719-ty.json` |
