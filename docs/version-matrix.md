...
普通 zip 或 source 快照 没有 `.git` 元数据，不能生成 commit matrix。这种场景只适合阅读顶层文档，不能作为版本锁定或完整链接测试依据。
...
| 2026-07-14 | `d8657fd` / 0.3.0 | `fb8fcc7` / 0.3.0 | `bdb9ff3` / 2.0.0 | workspace 2.0 已发布：删除 1.x 共享 命名空间 facade、旧 CLI 与环境变量 兜底。运行时 logger、artifact 契约 和活跃脚本统一为 owner-native 名称。三个仓库均完成 full gate、推送并与远端 `main` 同步。 | `docs/owner-native-namespace-release.json`、ADR-0002 |
