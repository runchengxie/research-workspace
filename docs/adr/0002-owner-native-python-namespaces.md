...
- `strategy-pipeline` 在 1.x 内集中提供唯一的 `cstree` Python facade 和 CLI alias。
...
1. alpha 和 portfolio 仓库不得出现 `src/cstree`。
2. strategy 仓库的 `src/cstree` 只能包含 `__init__.py` 与 `__main__.py`。
...
这是一组协调的 breaking migration。1.x compatibility facade 降低现有脚本的迁移峰值，
但不会继续充当实现所有者。2.0 删除前必须完成 consumer audit、替代入口文档、
focused tests、回滚说明和序列化对象审计。
