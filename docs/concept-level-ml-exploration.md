...
| H2 | 热榜排名变化包含预测信息（排名上升的概念次日跑赢排名下降的） | 计算 rank_momentum vs 次日概念成分股等权收益率的相关性 | 斯皮尔曼秩（Spearman rank）信息系数（IC） > 0.03 |
| H3 | 概念成分股的截面特征（市值中位数、换手率、波动率）能区分概念质量 | 用概念成分股的中位数特征做截面排名，看 IC | 均值排名（Mean rank）信息系数（IC） > 0.02 |
...
| LightGBM ranker | 非线性、交互项自动学习 | 样本量需求高 | 样本 > 10000 |
...
```
训练集：2024-10 至 2025-06（滚动前向（walk-forward），step=20 天）
...
- 每个概念按量价因子得分取 top N 只股票，不用 random 等权
...
| 概念间相关性导致过拟合 | 中 | 用滚动前向（walk-forward）+ 数据剔除（purging）/隔离窗口（embargo）严格防控 |
