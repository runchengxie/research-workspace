...
>
> 目标：用同花顺热榜 + 概念板块数据构建概念级别的 ML 截面排名能力，
> 替代当前的硬编码概念频率规则。最终实现"给定历史热点模式，预测明天哪些概念会跑赢"。
...
| 编号 | 假设 | 验证方式 | 通过标准 |
|------|------|---------|---------|
| H1 | 热榜概念有持续性信号（连续多天上榜的概念，次日成分股仍有正超额） | 计算"连续上榜 N 天"vs"首次上榜"的概念后续收益率差异 | 连续上榜组的平均超额 > 0 且显著 |
...
**探索不应跳过 H1-H4 的验证直接进入建模。** 如果 H1+H2 都不成立，热榜作为概念级预测信号的前提就不存在。
...
### 1.1 概念名称归一化
- **问题**：同花顺概念名不稳定（"AI算力" → "AI算力概念" → "算力概念"）
- **方案**：建立 concept_alias 映射表，模糊匹配 + 人工审核
- **产出**：`concept_id_mapping.csv`，每个 concept_id 对应一个标准名

### 1.2 概念成分股日频快照
- **问题**：kpl_concept_cons 只覆盖 394 天（2024-10 ~ 2026-05）
- **方案**：整合 kpl + dc_concept_cons（74天）+ ths_hot 概念字段做三方校验
- **产出**：`concept_constituents_daily.parquet`，每天每个概念 → 成分股列表

### 1.3 概念级收益率标签
- **方案**：概念成分股等权 T+1 open → T+2 open 收益率，取中位数（防异常值）
- **对比**：也可用市值加权、流通市值加权做敏感性分析
- **产出**：`concept_daily_returns.parquet`

### 1.4 概念归因穿透
- **方案**：对概念收益率做 Barra 风格因子归因，分离出"纯概念 alpha"
- **理由**：一个概念可能因为恰好暴露于动量因子而跑赢，这不需要概念信号
...
**建议**：先用 LinearRank（最少假设），如果特征工程证明有效但线性模型不够，再升级到 LightGBM。

### 3.2 标签选择

| 标签 | 定义 | 优点 | 缺点 |
|------|------|------|------|
| raw_return | 概念成分股等权 T+1→T+2 收益 | 直接可解释 | 受市场 beta 影响 |
| residual_return | 去 Barra 风格后的残余收益 | 纯 alpha | 需要额外归因层 |
| cross_sectional_rank | 当天横截面排名 | 天然截面比较 | 损失收益量级信息 |

**建议**：先用 raw_return，如果 beta 污染严重再切 residual_return。

### 3.3 验证框架

```
训练集：2024-10 ~ 2025-06 (walk-forward, step=20天)
测试集：2025-07 ~ 2026-05
...
当前问题：概念名 → kpl name 列匹配是简单的 substring 匹配。

改进方案：
- 用概念成分股的**截面量价特征**做子排序（把 cross-sectional-trees 的能力下沉到概念内）
...
| 热榜信号衰减过快 | 高 | 缩短持有期到 T+1 open→close，不做隔夜 |
