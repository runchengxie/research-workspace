...
| `legacy_rqdata_runtime.py` | RQData 和本地历史文件实现 | 已删除，见 internal PR #143 |

## 删除条件

以下条件已满足，模块已标记为 `archive` 并从 active source tree 删除：
...
4. internal 测试不再覆盖 RQData 运行时和本地 RQData 读取实现，模块已由 internal PR #143 删除。
5. clean-room 扫描确认公共仓库没有 provider SDK、凭证字段或私有策略内容。

## 当前判断

这部分代码的最终处理方向是归档和删除，不迁移到公共 `strategy-pipeline`。其中需要保留的通用符号规范
应由 `quant-execution-engine` 或 `market-data-platform` 提供 owner API，internal 不再保留兼容调用。
...
随后确认该模块已无生产调用方，并在 internal PR #143 中删除。
