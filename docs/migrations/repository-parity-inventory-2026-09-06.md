...
| `deep-learning-tick-data-prediction` | `quant-platform/microstructure` + `quant-research/microstructure` | full private baseline transferred; public extraction pending | local full suite green (`88 test files`); dedicated CI job added, remote result pending | 99 source files, 88 tests, 59 docs, 16 scripts, and 39 configs transferred privately | private baseline migrated; public/private split still pending | extract generic event/model abstractions while keeping labels, experiments, configs, and results private |
| `alpha-research` | `quant-platform/alpha` + private feature selections | full private alpha package transferred | local full suite green (`84 passed`); dedicated private CI job added, remote result pending | 155 source files, 84 tests, 33 docs, and 4 scripts transferred; parity manifest added | private alpha baseline migrated; public/private split still pending | classify reusable research machinery versus edge-bearing feature selections, labels, and model choices |
