...
| B2 | P1 | `in_progress` | data、alpha、portfolio、`strategy-app`、`strategy-pipeline`、workspace owners | 子模块边界重构（收口策略层与共享工具） | 2026-08-25 新增边界闭环：SA-4 已把 StyleReplica pipeline 的 `daily_clean.parquet`/`daily_basic.parquet`/`instruments.parquet`/`industry.parquet` 直读改为 `market-data-platform.research_views` published/current owner API，新增 ADR-0007 明确 StyleReplica 的 alpha / strategy policy / portfolio 三层 owner。ownership gate 已覆盖 legacy DeepSeek stability（1 + 16 个迁移债务定义）、session challenger（2 个）、three-arm contract freeze（0 增长）及 alpha 内 StyleReplica portfolio（14 个迁移债务定义），均设置 target 0。SA-13 的 `sha256_file`/`file_sha256` 定义已由 10 份收敛到 6 份，strategy-app 4→1、market-data-platform 2→1，并由 `tests/test_hash_utility_ownership.py` 冻结允许位置，剩余跨独立发布边界的 6 份需在不制造反向依赖的前提下继续评估。StyleReplica 旧 portfolio 实现仍待分批迁移，因此 B2 保持 `in_progress` |
