...
| B2 | P1 | `in_progress` | data、alpha、portfolio、`strategy-app`、`strategy-pipeline`、workspace owners | 子模块边界重构（收口策略层与共享工具） | StyleReplica 迁移已完成：alpha 只保留 signal/diagnostics，strategy-app 保留策略 policy，portfolio-backtester 提供通用 sleeve 构造，pipeline 只负责编排与导出。剩余 B2 工作仅包括跨发布边界的 6 份 hash helper 评估，因此不再登记 StyleReplica portfolio migration debt。 |
...
| M1 | P2 | `continuous` | 各仓 owner | 维护性预算收敛 | 机器账本真实未解决热点 80 条（登记册 182 条：97 条已降到阈值下、5 条已 cleared），受棘轮预算约束，大文件预算上限：data 41 / pipeline 28 / 组合回测 8 / 执行引擎 4 / alpha 5 / 顶层 3 / strategy-app 3。2026-08-18 顶层 `workspace_governance.py` 抽出 `workspace_governance_facades.py`，顶层大文件 5→4 下调，2026-08-19 将 `tests/test_next_open_to_high_research.py`(738) 拆为 `tests/_next_open_shared.py` 加 `tests/test_next_open_to_high_research_part2.py`（两文件均低于 500 行），顶层大文件 4→3 下调（PR #170 合并） |
...
import direction、source layout 与跨仓 private API 门禁均已通过。`workspace_import_boundaries.v3` 会解析 AST，
禁止 `strategy-app` 与 `strategy-pipeline` 从其他 owner 仓导入下划线模块或下划线符号，当前两者预算均为 0。
该项属于接口加固，不重新开启已完成的 R0 至 R6 物理拆分。
...
- 原则一：控制平面可以调用应用层公开 API。`strategy-pipeline` 负责 CLI、编排、外部调用、生产发布和
  ...
  完成指标是 pipeline 不复制策略计算内核，合法 import 无需清零。
...
- 原则三：共享工具不复制。SA-13 在 `strategy-pipeline` 内已改用
  `research_contracts.file_receipts.file_sha256`，其他仓的同类函数需重新按函数定义口径审计，不按搜索命中数计数，
  再决定 owner 和迁移批次。
...
| 研究到执行文件交接 | `complete` | `strategy-pipeline export-targets` 与 qexec 本地 dry-run 已通过基础契约验证 |
...
- G19（2026-08-25）B1/B2 边界门禁补洞：新增跨仓 private API AST 门禁并把 app/pipeline 现存命中清到 0，strategy-research 的 owner-private helper 测试迁回 alpha 后同样锁为 0，StyleReplica pipeline 改走 market-data-platform published research view，ADR-0007 冻结 StyleReplica 三层 owner，legacy DeepSeek stability、session challenger、three-arm 与 alpha StyleReplica portfolio 纳入 definition-level ownership ratchet，SA-13 hash 定义 10→6 并增加允许位置预算。历史 `submodule-boundary-refactor-checklist.md` 降为 historical，避免旧 SA-12/SA-14 结论继续被当作当前事实。
...
- G24（2026-08-28）StyleReplica 三层边界迁移完成：portfolio-backtester #49（`e589213`）提供通用 sleeve portfolio owner，strategy-app #48/#49（`82d0d43`）提供策略 policy 并锁定 alpha owner，alpha-research #39（`0cbf004`）移除最终持仓构造并改用 signal churn，strategy-pipeline #90/#91（`f2ab27b`）改为 canonical owner 编排并修正 turnover attribution owner。四个 gitlink 已同步，StyleReplica portfolio migration debt 清零。B2 仅保留 hash helper 的后续评估。
