...
| 任务 | 默认目标范围 | 只有这些情况才扩大 |
| --- | --- | --- |
| 通用数据、alpha、组合、回测、编排、执行能力 | `quant-platform` 中的直接 package | 修改公开 API、共享 contract、或下游测试失败 |
| 策略身份、策略逻辑、实验和私有配置 | `quant-research` 中的直接 package | 修改 publication contract 或需要验证 platform consumer |
