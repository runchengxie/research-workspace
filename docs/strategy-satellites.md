...
| --- | --- | --- | --- |
| `hot-sector-screener` | `market-intel` | `candidate_universe.json`、`signals.parquet` 和血缘（lineage） | 热点候选池、Numeric 排名和历史 challenger 输入 |
| `ai-stock-picker` | `market-intel` | `selection.json`、Prompt 与响应 sidecar、投递或观察回执（receipt） | DeepSeek shadow、稳定性实验和风险诊断 |
...
缺少这些正文时，只能保留为 retrospective 或 tombstone，不能补写成样本外（OOS）。
