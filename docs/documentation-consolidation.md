...
| DOC9 | 质量与维护债 | [maintainability-governance.md](maintainability-governance.md)、机器账本 | code health、code size、submodule refactor、noqa、R3 和 namespace migration 混合当前事实与历史计划 | 当前预算进入机器账本，完成的专项计划降级为 reference，审计快照进入 archive | 进行中（第二批完成维护性历史计划归集，strategy-pipeline 与 market-data-platform 的维护性审计快照已登记到 roadmap 专项账本入口） |
