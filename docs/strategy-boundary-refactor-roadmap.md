...
- `strategy-pipeline` 约有 90,768 行 Python 物理行，其中 `src` 约 50,592 行、测试约 34,586 行、脚本约 5,590 行。R3 删除 facade、R5 去重与 R4 迁移（DailyWatch20 六批、红利成长、D11-H5、热点 three-arm）后体量下降约 12,951 行，其中 `src` 下降约 7,339 行，控制面之外的运行时代码职责正在变薄。
...
- DailyWatch20 的 R4 迁移已推进 6 个 batch，红利与成长 ETF 动量、D11-H5 计算、热点板块 three-arm 已迁到 owner，次日开盘到最高价已彻底迁至 `strategy-research/experiments/next_open_to_high/`。热点板块其余子批次、DailyWatch20 剩余壳仍在 pipeline。
...
| R4 通用能力归位 | 进行中 | `date_utils` 已委托 `alpha-research`，DailyWatch20、红利成长、D11-H5、热点 three-arm 已迁 owner，继续按数据、alpha、组合、执行职责迁移热点板块其余子批次与 DailyWatch20 剩余壳 | pipeline 不再维护模型、通用统计、组合会计、成本或执行回放 |
...
| 3 | 热点板块 | `strategy-app` 加 `portfolio-backtester`，外部候选继续文件耦合 | 已推进：three-arm 组迁入 strategy-app，其余（v4_month、numeric_v2、holdings_overlay、session_challenger）待迁 |
...
下一次推进 R4 剩余切片：热点板块的 v4_month、numeric_v2、holdings_overlay、session_challenger 子批次，以及 DailyWatch20 剩余壳。红利成长、D11-H5、次日开盘已完成，DailyWatch20 六批与热点 three-arm 已迁 owner。每次恢复前先重新扫描 facade 消费者、冻结哈希和远端 `main`，不要沿用本页的静态计数代替代码事实。
