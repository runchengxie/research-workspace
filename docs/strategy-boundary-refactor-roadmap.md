...
| R2 pipeline 改名切换 | 待开始 | 更新依赖、Git pin、导入、类型配置、wheel smoke、活动文档和测试名称 | clean clone 只安装 `strategy-app` 0.2.x，活动代码不再导入 `research_apps` |
| R3 调用方改向 | 待开始 | 将 pipeline 内调用逐族改为 owner API，并在同一批次删除对应 facade | 登记的 39 个策略 owner facade 清零，且不新增替代兼容层 |
