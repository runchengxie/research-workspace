...
- `strategy-pipeline` 约有 93,693 行 Python 物理行，其中 `src` 约 53,034 行、测试约 35,068 行、脚本约 5,591 行。R3 删除 facade、R5 去重与 R4 DailyWatch20 迁移后体量下降约 10,026 行，其中 `src` 下降约 4,851 行，控制面之外的运行时代码职责正在变薄。
...
- DailyWatch20 的 R4 迁移已推进 6 个 batch：shadow 家族、叶子模块、PIT 特征、minute-campaign、long-horizon、slow-minute 执行已迁到 owner。热点板块、D11-H5、红利与成长 ETF 动量、次日开盘到最高价仍有策略计算或研究编排留在 pipeline。
...
| R4 通用能力归位 | 进行中 | `date_utils` 已委托 `alpha-research`，DailyWatch20 的 shadow 家族、叶子模块、PIT 特征、minute-campaign、long-horizon、slow-minute 执行已迁 owner，继续按数据、alpha、组合、执行职责迁移热点板块、D11-H5、红利成长、次日开盘 | pipeline 不再维护模型、通用统计、组合会计、成本或执行回放 |
...
下一次推进 R4 剩余切片：继续 DailyWatch20 剩余计算、热点板块、D11-H5、红利与成长 ETF 动量、次日开盘到最高价的策略计算迁移。DailyWatch20 的 shadow 家族、叶子模块、PIT 特征、minute-campaign、long-horizon、slow-minute 执行已迁 owner（10 个模块）。每次恢复前先重新扫描 facade 消费者、冻结哈希和远端 `main`，不要沿用本页的静态计数代替代码事实。
