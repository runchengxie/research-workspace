...
def test_module_groups_have_unique_active_ownership_and_evidence() -> None:
    ...
    assert sum(group["file_count"] for group in groups) == 93
...
def test_release_tools_are_archive_only() -> None:
    payload = _load_manifest()
    group = next(
        item
        for item in payload["module_groups"]
        if item["source_path"] == "src/strategy_pipeline_internal/release_tools"
    )
...
def test_internal_cli_is_archive_only() -> None:
    payload = _load_manifest()
    group = next(
        item
        for item in payload["module_groups"]
        if item["source_path"] == "src/strategy_pipeline_internal/cli"
    )
