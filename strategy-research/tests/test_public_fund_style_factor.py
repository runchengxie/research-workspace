...
def test_materialize_fund_portfolio_state_is_pit_and_formation_aligned() -> None:
    events = pd.DataFrame(
        {
            ...
            "fund_count_holding_stock": [10.0, 3.0, 7.0, 20.0],
            "fund_stk_float_ratio_sum": [1.5, 0.2, 1.0, 2.5],
    ...
    assert jan_a["fund_count_holding_stock"] == 10.0
    assert feb_a["fund_count_holding_stock"] == 7.0
    assert feb_a["fund_count_holding_stock_change"] == -3.0
    assert feb_a["fund_stk_float_ratio_sum_change"] == -0.5
    ...
    assert feb_b["fund_count_holding_stock"] == 3.0
    assert feb_b["fund_count_holding_stock_change"] == 0.0

    # C has no fund event after asset coverage starts, so it remains in the
    # cross-section as zero ownership instead of being silently excluded.
    assert feb_c["fund_count_holding_stock"] == 0.0
    assert feb_c["fund_stk_float_ratio_sum"] == 0.0
    assert feb_c["fund_count_holding_stock_change"] == 0.0

    # The March 10 disclosure must not leak into the February formation date.
    assert feb_a["fund_count_holding_stock"] != 20.0
