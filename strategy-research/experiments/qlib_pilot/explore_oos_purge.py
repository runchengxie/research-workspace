"""OOS 信号改进探索：purge/embargo 对 OOS IC 的影响。

假设：a_share 模型过拟合（train IC 0.14 vs OOS IC ~0）的部分原因是没有
purge/embargo 导致标签泄漏。本实验对比不同泄漏防护下的 OOS IC。

方法：同一真实面板、同一 XGB，滚动窗口，对比：
- 无防护（baseline，模拟当前配置）
- event_window purge（清洗训练里 label 与验证重叠的样本）

运行：
    uv run python explore_oos_purge.py
"""
...
LOGGER = logging.getLogger("oos_purge")
...
def run_window(panel, train_start, train_end, valid_start, *, purge: bool) -> float:
    ...
    if purge:
        # event_window purge：训练里 label 区间（trade_date 到 trade_date+20天）覆盖验证起点的样本去掉
        valid_start_ts = valid_start
        # label 在 [t, t+horizon) 生效，若 horizon 跨过 valid_start，说明用了验证期数据
        label_end = train["trade_date"] + pd.Timedelta(days=LABEL_HORIZON * 1.5)
        leak = label_end > valid_start_ts
        train = train[~leak]
...
def main() -> None:
    ...
    for t_start, t_end, v_start in windows:
        no_purge = run_window(panel, t_start, t_end, v_start, purge=False)
        with_purge = run_window(panel, t_start, t_end, v_start, purge=True)
        no_purge_ics.append(no_purge)
        purge_ics.append(with_purge)
        LOGGER.info(
            "w%d valid=%s no_purge=%.4f purge=%.4f",
            len(no_purge_ics), v_start.date(), no_purge, with_purge,
        )

    LOGGER.info("== summary ==")
    LOGGER.info("no_purge: mean=%.4f", np.nanmean(no_purge_ics))
    LOGGER.info("purge:    mean=%.4f", np.nanmean(purge_ics))
    LOGGER.info("improvement: %+.4f", np.nanmean(purge_ics) - np.nanmean(no_purge_ics))
