...
ensure_release_venvs() {
  ...
  if [[ -d "$release/quant-research" && -f "$release/quant-research/pyproject.toml" \
    && ! -x "$release/quant-research/.venv/bin/python" ]]; then
    ensure_project_venv "$release/quant-research" quant-research
  fi
}
